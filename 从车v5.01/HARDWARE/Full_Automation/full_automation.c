...
*/
u8 RFID_Card_Position = 1;								//RFID卡片位置标志位  默认位置为5
...
void AGV_return_Flag(u8 num)
{
	...
	S_Tab[1] = 0x21;
	S_Tab[2] = num;
	S_Tab[6]= (S_Tab[2]+S_Tab[3]+S_Tab[4]+S_Tab[5])%256;	 //第一组数据校验和
	S_Tab[7] = 0xBB;
	send_data_zigbee(S_Tab,8);  //AGV返回数据
	delay_ms(1000);
	S_Tab[0] = 0x55;
	S_Tab[1] = 0x21;
	S_Tab[2] = num;
...
void AVG_Car_End_Motion(u8 station)//AVG停靠的车库
{
	...
	Full_Left(L_sp);						//左转
	Full_Tracking(XJ_sp);					//循迹
	Full_Back(Back_sp,Back_mp-60);			//倒车入库适用于D、E、F车库
}
...
void Full_Motion()
{
	switch(mark)
	{
		...
		case 20://
			...
			Full_Left(L_sp);			//左转
			delay_ms(500);
			Full_RL_45(L_sp,1,30);					//右转45度
			mark =  25;
			break;
		case 25://
			Send_Debug_Info(G_Tab,6); 			// 上传调试信息
		    Send_Debug_Info((u8*)"\n",6); 		// 上传调试信息
		  	AGV_Infrared_Alarm(G_Tab);	   //发送得到红外报警数据
		    delay_ms(300);
			mark = 30;
			break;
		case 30://
